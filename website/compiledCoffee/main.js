...
(function() {
  var _currentUrl, activateIframe, appendToCache, defaultSettings, defaultURL, focusElement, getCookie, getSetting, getTopLocation, inCache, main, makeHash, makeIframe, minimizedSize, onScroll, onSettingsChanged, pollCurrentPage, prependToCache, removeFromCache, resetAllSettings, scroll, scrollAmount, sendMessageToIframe, setCookie, setLinks, setSetting, settingClamps, update, updateFromHash,
  ...
  setLinks = function(url) {
    ...
    if (getSetting('show-reddit-link') && hasRedditDiscussion(url)) {
      return $('#reddit-link').attr('href', getRedditDiscussionUrl(url)).show();
    } else {
      return $('#reddit-link').hide();
    }
  };
