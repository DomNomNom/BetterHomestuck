...
(function() {
  var baseURL, beginEnd, bisectBelow, comicNum, comicNumRanges, containsPageNumber, findComicNum, from, i, isA6A5A1X2COMBO, isBegin, isEnd, j, k, len, makeUrl, next, obj, oneWayLinks, pad6, pageChain, pageNum, prev, ref, specialNextLinks, specialPageChains, specialPages, specialPages_reverse, specialPrevLinks, to, url;
  ...
  window.pageRequiresKeyboard = function(url) {
    ...
    if (!(pageNum in window.flashPages)) {
      return false;
    }
    return window.flashPages[pageNum] & 2;
  };
  ...
  window.hasRedditDiscussion = function(url) {
    pageNum = getPageNumber(url);
    return (redditDiscussion_min <= pageNum && pageNum <= redditDiscussion_max);
  };

  bisectBelow = function(bot, top, targetPageNum) {
    var mid, midPage;
    if (bot === top) {
  ...
  window.getRedditDiscussionUrl = function(url) {
    var redditExtension;
